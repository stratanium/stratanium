...
public:
    ...
    // Stratanium Specific WalletInitInterface InitCoinJoinSettings
 ...
 * between strataniumd, and stratanium-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("stratanium-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-sporkaddr=<strataniumaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by Stratanium Platform, to the specified username.", false, OptionsCategory::SMARTNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/Raptor3um/stratanium>";
    const std::string URL_WEBSITE = "<https://stratanium.com>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("stratanium-loadblk");
...
/** Sanity checks
 *  Ensure that Stratanium Core is running in a usable environment with all
...
{
    // Make sure only a single Stratanium Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if Stratanium Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Stratanium Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 10c: schedule Stratanium-specific tasks
