...
const char * const BITCOIN_CONF_FILENAME = "stratanium.conf";
const char * const BITCOIN_PID_FILENAME = "strataniumd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "strataniumd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\StrataniumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\StrataniumCore
    // Mac: ~/Library/Application Support/StrataniumCore
    // Unix: ~/.strataniumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "StrataniumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/StrataniumCore";
#else
    // Unix
    return pathRet / ".strataniumcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty stratanium.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Stratanium Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Stratanium Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Stratanium Core developers";
    }
