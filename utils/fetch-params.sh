...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/StrataniumParams"
else
    PARAMS_DIR="$HOME/.stratanium-params"
fi
...
DOWNLOAD_URL="https://github.com/Raptor3um/stratanium/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with STRA_DISABLE_SOMETHING=1
STRA_DISABLE_WGET="${STRA_DISABLE_WGET:-}"
#STRA_DISABLE_IPFS="${STRA_DISABLE_IPFS:-}"
STRA_DISABLE_CURL="${STRA_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$STRA_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$STRA_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$STRA_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Stratanium parameters!
...
main() {
    ...
    cat <<EOF
STRA - fetch-params.sh

This script will fetch the Stratanium SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Stratanium SNARK parameters. Note that it is
