...
# stratanium_hash
RUN git clone https://github.com/raptor3um/stratanium_hash
RUN cd stratanium_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} stratanium
RUN useradd -u ${USER_ID} -g stratanium -s /bin/bash -m -d /stratanium stratanium
...
RUN mkdir /stratanium-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /stratanium-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /stratanium-src
